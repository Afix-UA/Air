(() => {
    ...
    (() => {
        ...
        function initializeDatepicker(pickerElement) {
            const controlsNext = pickerElement.parentElement.querySelector(".calendar-controls-next");
            const controlsPrev = pickerElement.parentElement.querySelector(".calendar-controls-prev");
            let selectedDate = new Date;
            const pickerInstance = datepicker_min(pickerElement, {
                ...
                overlayButton: "Відправити",
                overlayPlaceholder: "Введіть рік у форматі 4 цифри",
                startDay: 1,
                position: "bl",
            ...
            if (controlsNext && controlsPrev) {
                controlsNext.addEventListener("click", (e => {
                    e.preventDefault();
                    selectedDate.setDate(selectedDate.getDate() + 1);
                    if (pickerInstance) pickerInstance.setDate(selectedDate);
                }));
                controlsPrev.addEventListener("click", (e => {
                    e.preventDefault();
                    selectedDate.setDate(selectedDate.getDate() - 1);
                    if (pickerInstance) pickerInstance.setDate(selectedDate);
                }));
        ...
        function initPickerMin() {
            const pickerMin = document.querySelector("[data-datepicker-min]");
            if (pickerMin) {
                const pickerInstance = initializeDatepicker(pickerMin);
                modules_flsModules.datepicker = pickerInstance;
            }
        }
        function initPickerMax() {
            const pickerMax = document.querySelector("[data-datepicker-max]");
            if (pickerMax) {
                const pickerInstance = initializeDatepicker(pickerMax);
                modules_flsModules.datepicker = pickerInstance;
            }
        }
        initPickerMin();
        ...
        function initSlidersEnjoy() {
            if (document.querySelector(".enjoy__slider")) new swiper_core_Swiper(".enjoy__slider", {
                modules: [ Navigation, Autoplay ],
                observer: true,
                observeParents: true,
                slidesPerView: 1,
                ...
                slideActiveClass: "enjoy-slide-active",
                effect: "fade",
                autoplay: {
                    delay: 3e3,
                    disableOnInteraction: false
